  ...
  if (!conf.fail()) {
    ...
    while (std::getline(conf, line)) {
      ...
      if (std::regex_match(line, r_empty)) {
        ...
        conf_db.emplace_back(
            ConfEntry({i, m[1].str(), boost::trim_copy(m[2].str()), false}));
      } else {
...
template <typename T>
void config_overlay(const boost::program_options::variables_map &vm,
                    const char *name, T &dst) {
  if (vm.count(name) == 0) {
...
int main(int ac, char *av[]) {
  ...
  desc.add_options()("help,h", "produce help message")(
      ...
      "log file (only for debugging)")("cmd", po::value<std::string>(&cmd_file),
                                       "Unix-socket for command interface")(
      "port,p", po::value<unsigned int>(&listen_port_number),
