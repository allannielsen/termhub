...
template <unsigned BASE> bool lexical_convert(const char c, unsigned &v);

template <> inline bool lexical_convert<2>(const char c, unsigned &v) {
  if (c >= '0' && c <= '1') {
    v = c - '0';
    return true;
  }
...
template <> inline bool lexical_convert<8>(const char c, unsigned &v) {
  if (c >= '0' && c <= '7') {
    v = c - '0';
    return true;
  }
...
template <> inline bool lexical_convert<16>(const char c, unsigned &v) {
  if (c >= '0' && c <= '9') {
    v = c - '0';
    return true;
...
bool parse(const char *&b, const char *e,
  ...
  if (digit_cnt == 0)
    goto Error;
  if (MIN_DIGITS != 0 && digit_cnt < MIN_DIGITS)
    goto Error;
  if (MAX_DIGITS != 0 && digit_cnt > MAX_DIGITS)
    goto Error;
